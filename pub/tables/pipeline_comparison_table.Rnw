...
<<summary_table, results=tex,echo=FALSE>>=
...
<< ec_vcf_table, results=tex,echo=FALSE>>=
...
<< lm_vcf_table, results=tex,echo=FALSE>>=
strCaption <- "\\textbf{L. monocytogenes Positions Pipeline Comparison} Characteristics of variant calls for different bioinformatic pipelines."
print(xtable(vcf_tables_lm, 
             digits=2, 
             caption=strCaption, label="Pipeline Comparison LM"),
      include.rownames = FALSE,
      tabular.environment='longtable', 
      caption.placement="top",)
@
