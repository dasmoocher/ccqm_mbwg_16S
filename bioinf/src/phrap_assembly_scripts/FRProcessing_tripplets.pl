...
my $mdirName = readInDir("middle sequence");
...
my @middleFileNames = ();
foreach my $i (@middleFiles)
{
       my $filename = basename($i);
       push (@middleFileNames, $filename);
}
...
if  ($fileNamingType eq "Lindsay")  #
{
  ...
  foreach (@forwardFiles)
  {
    ...
    #changed for my reads nate olson 4/22/2013
    my $barcode = substr($fseqname, 0, 10);

    my @grepRev = grep(/^$barcode/, @revFileNames);
    my @grepMiddle = grep(/^$barcode/, @middleFileNames);

    my $arrayRSize = @grepRev;
    my $arrayMSize = @grepMiddle;

    if  (($arrayRSize > 1) or ($arrayMSize > 1))
    {
     print "This script assumes only one reverse or middle read for a given barcode. I have just found more than one matching sequence for for $barcode. Exiting\n.";
     exit;
    }

    elsif (($arrayRSize < 1) or ($arrayMSize < 1))
    {
     print "Warnings: there is a missing read partner (reverse or middle read) for the forward sequence with barcode $barcode\n";
     next;
    }
    ...
    { #implicit that your size = 1
     ...
     my $mseqname = $mdirName . "/" . $grepMiddle[0];
...
#change the name of the forward sequence - in this case, make a copy and use this as the F seq in the assembly.
	...
	my $mexp = $grepMiddle[0]; $mexp =~s/ab1/exp/;
...
#check for good sequences. Reports back to user.
        ...
        my $warning =  "$fseqname has $seqlengths[0] quality bases.\n$grepMiddle[0] has $seqlengths[1] quality bases\n$grepRev[0] has $seqlengths[2] quality bases.\n";
        print $warning;
        print LOG $warning;
        close LOG;
